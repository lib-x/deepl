...
const (
	deepLProRpcServer = "https://api.deepl.com/jsonrpc"
)
...
func (c Client) Translate(sourceLanguage, targetLanguage, textToTranslate string) (jsonRpcResponse *rpc.JsonRpcResponse, err error) {
	var (
		requestServer = deepLProRpcServer
		headerIter    = headers.DeeplProHeaderIter(c.opt.DlSession)
	)
	if sourceLanguage == "" {
	...
	if c.opt.TagHandling != "" {
		postData.Params.TagHandling = c.opt.TagHandling
	}
	// set id
	...
	client := &http.Client{}
	if transport := kits.BuildHttpTransportWith(c.opt); transport != nil {
